...
`SOCK_STREAM` fallback on Linux, and native fallback on GitHub's current Apple
Silicon macOS runner. Every formula test initializes and indexes a v2 inbox. On
...
Homebrew's more restrictive formula-test sandbox. Older macOS releases and
Intel remain best-effort compatibility targets rather than continuous gates.
